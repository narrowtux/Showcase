		...
		AssistantPage page = new AssistantPage(this) {
			{
				...
				String text = "";
				String itemName = Showcase.getName(showcase.getMaterial(),
						showcase.getData());
				String exchangeName = Showcase.getName(extra.getExchangeType(),
						extra.getExchangeData());
				text += "Get " + ChatColor.YELLOW + extra.getExchangeRateLeft()
						+ " " + itemName + ChatColor.WHITE;
				text += " for " + ChatColor.YELLOW
						+ extra.getExchangeRateRight() + " " + exchangeName
						+ "\n";
				int playeramount = player.getAmountOfType(
						extra.getExchangeType(), extra.getExchangeData());
				text += "You have got " + ChatColor.YELLOW + playeramount + " "
						+ exchangeName + "\n";
				text += "The showcase has got " + ChatColor.YELLOW
						+ extra.getItemAmount() + " " + itemName + "\n";
				text += "Type " + ChatColor.YELLOW + "0 or go away"
			...
			public AssistantAction onPageInput(String text) {
				...
				player.addItems(showcase.getMaterial(), showcase.getData(),
						amount);
				player.remove(extra.getExchangeType(), extra.getExchangeData(),
						exAmount);
				extra.addExchanges(exAmount);
				extra.addItems(-amount);
				sendMessage(formatLine("You got " + amount + " " + itemName
						+ " for " + exAmount + " " + exchangeName));
				ShowcasePlayer owner = ShowcasePlayer.getPlayer(showcase
						.getPlayer());
				owner.sendMessage(player.getPlayer().getName() + " got "
						+ amount + " " + itemName + " for " + exAmount + " "
						+ exchangeName);
